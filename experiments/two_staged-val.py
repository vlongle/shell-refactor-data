...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    handlers=[RichHandler(rich_tracebacks=True, markup=False, show_time=False, show_path=False),
                              # handlers=[
                              # log to file
                              logging.FileHandler("log_val_instead_test.txt", mode="w")],
                    )
...
def main():
    ...
    # sender_cfg.dataset.train_size = size * 4
    sender_cfg.dataset.train_size = size * 4
    # sender_cfg.router.batch_size = size * 4 * 2
    ...
    # 0 and 1, 4 and 9 are hard to establish!
    receiver.ll_dataset.perm = torch.tensor([0, 1, 4, 9])
    sender.ll_dataset.perm = torch.tensor([0, 1, 4, 9])
    ...
    receiver.learn_task(0, metric="val_acc", record_name="val_instead.csv")
